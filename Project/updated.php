...
if(isset($_SESSION['name']))
{
  ...
  $buttonref="adminactions.php";
    ...
    if(isset($_POST['submit']))
	{
        ...
        $sql="UPDATE tool SET name_en ='$name_en', name_ar='$name_ar', description='$description', category='$category'  WHERE id = '$id'";
        $result=mysqli_query($conn, $sql);

        //updating image and move the other to differnt folder
        if(file_exists($_FILES["img"]["tmp_name"])){
        $dst="./images/myway/";
        $src="./images/updated/updated_";
        $newfilename= $id.".jpg";
        rename($dst.$newfilename,$src.$newfilename);
        move_uploaded_file($_FILES["img"]["tmp_name"], $dst.$newfilename);
        //updating image and move the other to differnt folder
      }
    }
					...
					<div class="row">
						<div class="col-md-12 mb-0">
							<a href="indexx.php">Home</a> <span class="mx-2 mb-0">/</span> <strong
								class="text-black">Updated</strong>
						</div>
					</div>
				</div>
			</div>
