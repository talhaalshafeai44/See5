...
{
    $data = $_GET['data'];
  $sql2="UPDATE messages SET status = 'read' WHERE id ='$data'";
          ...
          <div class="main-nav d-none d-lg-block">
            <nav class="site-navigation text-right text-md-center" role="navigation">
              <ul class="site-menu js-clone-nav d-none d-lg-block">
                ...
                <li class="active"><a  href="inbox.php" class="icons-btn d-inline-block bag">
...
<?php
	...
	if(isset($_SESSION['name']))
	{
		...
		<th> Email </th>
		<th> Subject </th>
		<th class='en'> Message </th>
		<th>Status</th>
		<th>Action</th>
		</tr>";
		$sql = "SELECT * FROM messages  ORDER BY status DESC ";
    ...
    if ($result->num_rows > 0)
	{
       while($row = $result->fetch_assoc())
	   {
		   ...
		   <td class='ar'>" .$row['email']."</td>
		   <td class='des'>".$row['subject']." </td>
        <td  class='en'> ".$row['message']."</td>
		   <td class='cat'>".$row['status']."</td>
		   <td class='link'>
		   <a title='Mark as read' href='./inbox.php?data=$row[id]' >
		   <img src='images\d.ico' alt='icon' height='50px' width:'50px'>
		   </img>
		   </a>
