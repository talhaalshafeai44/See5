...
$last_id=mysqli_query($conn,"SELECT id FROM `tool` ORDER BY id DESC LIMIT 0 , 1" ); //from 30 +1 = 31
$row=mysqli_fetch_assoc($last_id);
$nextid=$row['id']+1; //31

	//image
    $dst="./images/myway/";//Destination
    $newfilename= $nextid.".jpg";//image name with jpg ex:31.jpg
    move_uploaded_file($_FILES["img"]["tmp_name"], $dst.$newfilename); //img the id of uploded image in addtool.php
    //image


$sql="INSERT INTO tool(id,name_en,name_ar,description,category) VALUES ('$id','$name_en','$name_ar','$desc','$category')";
